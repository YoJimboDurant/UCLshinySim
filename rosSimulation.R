...
bootplot1 <- ggplot(bootUCL, aes(x=factor(N), y=value, colour=factor(variable))) +geom_boxplot() + scale_y_log10() + geom_abline(intercept=tesit$pop.mean[1])
(bootplot1 <- bootplot1 + geom_abline(intercept=log10(tesit$pop.mean[1]), slope=0))
#coverages
...
(bootplot2 <- ggplot(coverageUCLboot, aes(x=factor(N), y=coverage, colour=variable, group=variable)) + geom_point() + geom_line() +xlab("Sample Size") + ylab("Coverage"))
(bootplot3 <- ggplot(coverageUCLboot, aes(x=factor(N), y=coverage5percent, colour=variable, group=variable)) + geom_point() + geom_line())
(bootplot4 <- ggplot(coverageUCLboot, aes(x=factor(N), y=coverage10percent, colour=variable, group=variable)) + geom_point() + geom_line())
